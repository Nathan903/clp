...
- **IP addresses** — visible during network communication but **not logged or stored** in the telemetry backend
- **Credentials and secrets** — no passwords, API keys, or connection strings
- **Hostnames** — no server or container hostnames
- **Local timezone** — all timestamps are UTC; no timezone information is collected as it could reveal geographic region
