...
logs are additionally written to `<CLP_LOGS_DIR>/<component_name>.log`. This directory mounts to the host via `CLP_LOGS_DIR_HOST` (defaults to `./var/log`).
  :::{note}
  For Rust services, setting `CLP_LOGS_DIR` enables file logging with hourly log rotation.
  Both stdout and the rolling file writer use lossless non-blocking I/O via a background
  worker thread, so log writes do not block the service's async tasks.
  :::
