...
class PathsToCompressBuffer:
    def __init__(
        ...
        self.__target_uncompressed_size: int = clp_io_config.output.target_uncompressed_size
        self.__file_size_to_trigger_compression: int = (
            clp_io_config.output.target_uncompressed_size * 2
        )
    ...
    def add_files(self, target_num_archives: int, target_uncompressed_size: int, files):
        ...
        while len(groups) > 0:
            ...
            while True:
                ...
                if partition.get_total_file_size() < target_uncompressed_size:
                    break
    ...
    def __partition_and_compress(self, flush_buffer: bool):
        if not flush_buffer and self.__total_file_size < self.__target_uncompressed_size:
        ...
        if self.__maintain_file_ordering:
            ...
            if self.__total_file_size >= self.__target_uncompressed_size:
                file_ix = 0
                for file_ix, file in enumerate(self.__files):
                    ...
                    if partition.get_total_file_size() >= self.__target_uncompressed_size:
            ...
            while len(groups) > 0:
                ...
                if partition.get_total_file_size() >= self.__target_uncompressed_size:
                    ...
                    if (
                        not flush_buffer
                        and self.__total_file_size < self.__target_uncompressed_size
                    ):
